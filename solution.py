...
class BAProblem(search.Problem):
    ...
    def check(self, sol):
        ...
        for i, (ui, vi) in enumerate(sol):
            if ui < self.vessels[i][0]: #Verifica se o barco já chegou à hora de descarregar
                return False
            if vi + self.vessels[i][2] > self.S: #Verifica se o barco cabe no cais
                return False
            for j in range(self.vessels[i][2]): #Preenche a matriz de ocupação               
